...
class AbstractRepository
{
    ...
    {
        return $this->modelAnalizer->getTableName(
    ...
    {
        return $this->modelAnalizer->getIdColumnName(
            $this->getExampleModel()
        );
    }
...
//               dump($modelClass); exit;
    ...
    protected function createModelByArray(array $data)
    {
        ...
        $this->modelAnalizer->setId($model, $id);

        $tableFields = $this->modelAnalizer->getTableFields(
            $this->getExampleModel()
        );
        foreach ($tableFields as $propertyName => $tableField) {
            ...
            if (!is_null($propertyValue)) {
                $this->modelAnalizer->setProperty($model, $propertyName, $propertyValue);
            }
        }

        $relations = $this->modelAnalizer->getRelations(
            $this->getExampleModel()
        );
        foreach ($relations as $propertyName => $tableFieldName) {
            $this->modelAnalizer->setRelation($model, $propertyName, $tableFieldName);
...
//        ];
        ...
        $query = "SELECT * FROM $tableName $where LIMIT $limit";
